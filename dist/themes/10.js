...
class ThemeTen {
    ...
    buildEmbed(game, data, test) {
        const button = game.urls.client
            ? game.store === 'steam'
                ? `${__1.Localisation.text(data, '=open_in_browser')}: [https://s.team/a/${game.urls.org.split('/app/')[1].split('/')[0]}](${game.urls.browser})\n${__1.Localisation.text(data, '=open_in_steam_client')}: ${game.urls.client}`
                : `${__1.Localisation.text(data, '=open_in_browser')}: [${game.urls.org}](${game.urls.browser})\n${__1.Localisation.text(data, '=open_in_epic_games_client')}: <${game.urls.client}>`
            : `[${__1.Localisation.text(data, '=open_in_browser')}](${game.urls.default})`;
